...
class fetch_xshells(rfm.RunOnlyRegressionTest):
    ...
    descr = "Fetch XSHELLS repository and checkout cheese-miniapp-fp32 branch"

    repo_url = "https://gricad-gitlab.univ-grenoble-alpes.fr/schaeffn/xshells.git"
    branch = variable(str, value="cheese-miniapp-fp32")

    executable = "git clone"
    executable_opts = [f"-b {branch}", "--recurse-submodules", f"{repo_url}"]
